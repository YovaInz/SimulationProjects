...
public class bufon extends JPanel {
  private static Scanner leer = new Scanner(System.in);
  private float n; // Almacena el número de agujas
  private static double hip;
  private static int skipAguja = 0, skip = 0, dardos = 0;

  public bufon(int n) {
    this.n = n; // Asigna el número de agujas a la variable de instancia
    setBorder(new EmptyBorder(5, 5, 5, 5));
    setBackground(Color.white);
    setPreferredSize(new Dimension(800, 1000)); // Establece el tamaño preferido del panel
  }
  ...
  public void paint(Graphics g) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      dibujaAguja(g2d, coordenadas[0], coordenadas[1], coordenadas[2], coordenadas[3]);

      if (coordenadas[1] / 200 != coordenadas[3] / 200) {
        contador_a++;
      }
    }
    // Grafica de pastel
    ...
    double comprobante = (2 * n) / contador_a;
    if (skipAguja == 2) {
      frame.setVisible(true);
      resultados(contador_a, comprobante, n);
    }
    skipAguja++;
  ...
  private int[] generarCoordenadasAleatorias() {
    ...
    do {
      ...
      longitud = (int) (Math.sqrt(Math.pow(x2 - x1, 2) + Math.pow(y2 - y1, 2)));
    } while (longitud != 200);
  ...
  public static void resultados(double contador_a, double comprobante, double n) {
    System.out.println("Las agujas que chocan son:" + contador_a);
    System.out.println("2*" + n + "/" + contador_a + "=" + comprobante);
  }
  ...
  public static void agujas() {
    ...
    int n = leer.nextInt(); // Cierra el Scanner después de usarlo

    bufon panel = new bufon(n);
    mostrarPanel(panel);
  }

  private static void mostrarPanel(JPanel panel) {
    JFrame frame = new JFrame("Simulación de agujas de bufon");
    frame.add(panel);
    frame.pack();
    frame.setVisible(true);
  }
